...
proto._clearUnnecessaryModifiers = function (originalDoc) {  //add option to pass originalDoc back to function when it is called for a nested class or array
  old = old || false;
  var doc = this;
  var Class = doc.constructor;

  if (!originalDoc && doc._id && Class.getCollection()) {
  ...
  _.each(fields, function (field, fieldName) {
    ...
    if (
      ...
      if (originalDoc[fieldName]) {// Check if nested field exists in original parent document
        var nestedDoc = doc[fieldName];
        nestedDoc._clearUnnecessaryModifiers(originalDoc[fieldName]);
      }
      // Clear modifiers.
      ...
      _.each(nestedDocs, function (nestedDoc, nestedDocIndex) {
        // Clear modifiers.
        if (originalDoc[fieldName][nestedDocIndex]) { // Check if nested field exists in original parent document
          nestedDoc._clearUnnecessaryModifiers(originalDoc[fieldName][nestedDocIndex]);
        }
      });
    } else {
